...
function setBackground(weatherIcon) {
    ...
    const imageUrl = bgImages[weatherIcon]
...
async function fetchWeather({lat, lon, city}) {
    ...
    try {
        ...
        const weatherCelsiusUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${OWNS_API_KEY}&units=metric`;

        // const weatherFahrenheitUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${OWNS_API_KEY}&units=imperial`;

        const forecastCelsiusUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&appid=${OWNS_API_KEY}&units=metric`;

        // const forecastFarenheitUrl = `http://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&appid=${OWNS_API_KEY}&units=imperial`;

        const airQualityUrl = `https://api.openweathermap.org/data/2.5/air_pollution?lat=${latitude}&lon=${longitude}&appid=${OWNS_API_KEY}`;

        const [weatherCelsiusUrlresponse, forecastCelsiusUrlresponse, airQualityUrlresponse] = await Promise.all([
            fetch(weatherCelsiusUrl),
            fetch(forecastCelsiusUrl),
            fetch(airQualityUrl),

        ])

        if ([weatherCelsiusUrlresponse, forecastCelsiusUrlresponse, airQualityUrlresponse].some(res => {
            !res.ok
        })) {
            throw new Error(`Failed to fetch all weather data with HTTP error! Status: ${res.status}`)
        }

        const weatherCesiusData = await weatherCelsiusUrlresponse.json();
        const forecastCelsiusData = await forecastCelsiusUrlresponse.json();
        const airQualityData = await airQualityUrlresponse.json();

        updateUI(weatherCesiusData, forecastCelsiusData, airQualityData)
...
function updateUI(weather, forecast, aqi) {
    ...
    currentTemp.textContent = `${Math.round(weather.main.temp)} ${unit}`;
    ...
    feelsLike.textContent = `${Math.round(weather.main.feels_like)} ${unit}`
...
function Weatherwarning(id) {
    ...
    const data =  weatherWarningsData[id].warningLevel;
    ...
    dailyForecasts.forEach(day => {
        ...
        card.innerHTML = `
            ...
            <div class="flex flex-col">
                    <div class="p-4 mt-1 rounded-2xl backdrop-blur-xl flex flex-col">
